...
clear_tmp() {
	test "${STORE_DIR:-}" && rm -rf "$_" || true
}

trap '{ clear_trap; clear_tmp;} 2>&-' EXIT
################
# LIBRARY
################
...
declare -r OBABEL=$(type -P obabel)
...
################
_00_checkToolLibDir() {
	for x in ${REDUCE} ${BABEL} ${BALLOON} ${ENVA} ${SCWRL4} ${ISKEW} ${RMSD} ${MPIRUN}  ${MOL_TO_PARAM} ${RLD_MPI} ${OBABEL}; do
...
_01_mpiRLD() {
	...
	$OBABEL $_03_ligand_sdf -O $_04_conformer_sdf --ff ${MFF_LIB%%.*} --confab --conf 100000
	if [ $(wc -c $_04_conformer_sdf | awk '{print $1}') -eq 0 ]; then
...
_09_compressData() {
	...
	tar cfz ${SAMPLE_NAME}_scwrl.tar.gz ${SAMPLE_NAME}
	cp ${SAMPLE_NAME}_scwrl.tar.gz $_output
}

_10_rmWorkDir() {
	rm $STORE_DIR/../${SAMPLE_NAME}_scwrl.tar.gz
	rm -r $STORE_DIR
}
#####################
# MAIN
